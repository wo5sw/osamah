...
## 4) كيف يعمل التوجيه (من الطرف للطرف)
```
Users.merchant (رقم القسم)
   └─ login/getSections يُرجعه ──▶ أداة التاجر تحقن «رقم التاجر: <الرقم>» في التدوينة عند النشر
        └─ المتجر يقرأ الرقم مع المنتج ──▶ زر «اشترِ الآن» يفتح wa.me/<رقم تاجر القسم>
```
لو لم يُضبط رقم القسم في `Users` يرجع الطلب تلقائيًا للرقم الرئيسي `CONFIG.whatsapp`.
